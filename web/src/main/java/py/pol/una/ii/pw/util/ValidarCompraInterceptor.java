...
@ValidarCompra
@Interceptor
public class ValidarCompraInterceptor {
    ...
    public Object verificaLogin(InvocationContext ctx) throws Exception {
        ...
        try {
            Usuario user = servicio.findByTokenCompra(token);
            if(user != null){
                System.out.println("El usuario tiene permisos para realizar compras");
                return ctx.proceed();
            }else{
                System.out.println("No se encuentra el token con el rol compra en la BD, se debe redireccionar al LOGIN");
                return null;
            }
        } catch (Exception e) {
